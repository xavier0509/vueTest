...
//第一个参数buts是该页面所有按钮的class
//第二个参数是设置默认的把焦点落在哪个class或者id上
//第三个参数是获取焦点之后的样式
//第四个参数是
//第五个参数是
//第六个参数是自定义的一些方法
...
coocaakeymap.prototype.handleClick = function(obj){
	this.setFocus($(obj));
	this.curLink.trigger("item-click");
};
coocaakeymap.prototype.handleEvent = function(eventName){
	
	var vue = vueDomRef.get(this.curLink.get(0));
	if(null != vue){
		vue.$emit(eventName, this.curLink);
	}
};
coocaakeymap.prototype.keyHandler = function(_this, ev){
	...
	if(ev.isPropagationStopped() == false){
		switch(curKey){
			...
			case 40: // down
				...
				break;
			case 13: // enter
				_this.curLink.trigger("item-click");
				_this.handleEvent("item-click");
				break;
			case 8:// 遥控器删除
				_this.iscmd = true;
				_this.cmd = [];
	...
	if(lastLink != _this.curLink){
		lastLink.trigger("item-blur");
		// _this.curLink.trigger("item-focus");
		_this.handleEvent("item-focus");
	}
